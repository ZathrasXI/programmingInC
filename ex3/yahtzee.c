...
bool isfullhouse(const int d[NUMDICE]){
    int first_die = d[0];
    int other_number;
    int counter_other_number = 0;
    int counter_first_die = 0;

    for (int i = 0; i < 6; i++){
        if (d[i] != first_die){
            other_number == first_die;
            break;
        }
    }
    ...
    if (counter_first_die + counter_other_number == 5){
        return true;
    }
    return false;
}
